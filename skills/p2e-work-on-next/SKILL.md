...
Execute the shared orchestrator workflow exactly. Codex has no nested Agent tree or dynamic Workflow tool — use the documented **sequential fallback** in `## Cross-platform fallback`: per story, brief → implement inline (adaptive skill matrix still applies) → verify via the adaptive fix loop (per `## Verify gate` in policy; same 6-round/stall/oscillation exits) → commit + PR → tiered review; track progress with `update_plan` (one entry per story). Stories end at `IN_REVIEW`; never cut a release.
