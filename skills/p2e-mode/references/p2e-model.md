...
| **Criterion** | One testable AC; VERIFIER and AUDITOR assess separately | Bulk-approvable |
...
- Roles: `VERIFIER` | `AUDITOR`. Verdicts: `PASS | FAIL | BLOCKED` (auditor: PASS/FAIL). Absence / `NOT_TESTED` = unassessed.
- Agents write via `criteria op=propose`. `op=verdict` / `op=toggle` are not for agents.
- Auditor lists with `viewer_role=AUDITOR` — verifier block omitted.
