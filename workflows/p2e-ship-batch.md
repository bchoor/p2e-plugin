...
12. Each story completes Phase B at `status=IN_REVIEW` with recorded AC verdicts (via `criteria op=verdict`) and a clean story-log trail (DEVIATIONS entry + VERIFICATION entry). Stories that hit two-strike escalation are at `status=BLOCKED` — those are **excluded from Phases C–E**, their task is kept `in_progress` with a "BLOCKED — needs human triage" status-line note, and they're surfaced in the final roll-up under "Blocked".

### Phase C — 360° verify per story

13. For each story now at `IN_REVIEW`, call `/p2e-verify-story <story_id> --report` (standalone mode with report). The command bind-checks `.p2e/project.json`, fetches the story via `mcp__p2e__stories op=get`, brings the dev server up reliably (detached `nohup` launch with `lsof` port verification), reproduces every AC via a browser-driver MCP (`mcp__chrome-devtools__*` preferred, `mcp__claude-in-chrome__*` fallback) or curl for backend-only ACs, records per-AC verdicts via `mcp__p2e__criteria op=verdict` with screenshot evidence uploaded via `mcp__p2e__story_assets op=upload_url` (signed-URL path — see `## Screenshot evidence upload` in `workflows/p2e-policy.md`), and emits a self-contained rich-HTML report at `docs/feat-<topic>/uat-results/results.html` (preferred) or `.claude/uat-results/<story-id>/results.html` (fallback when no feature folder is bound). Pass `--artifacts-dir=<path>` per-batch to redirect output if needed. The tracker (detail panel + map badge) now shows live UAT state directly from the recorded verdicts — the HTML report is the secondary artifact for human review.
...
- Phase B inherits work-on-next's self-enforced MCP status discipline (`workflows/p2e-work-on-next.md` Phase 2a status flip — the `OPEN → IN_PROGRESS` transition that must complete before story-leads are spawned). This is enforced by the workflow on every platform; there is no `PreToolUse` hook backstop.
