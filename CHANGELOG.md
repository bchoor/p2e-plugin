...
Also tightens the shared first-turn briefing with an explicit **implementer deviation-reporting contract**: when implementation reveals the spec is wrong, incomplete, or conflicts with reality, the implementer must emit a `SCOPE_CHANGE` or `DECISION` story-log entry **before** making the deviating change. `SCOPE_CHANGE` and `DECISION` are no longer human-authored only — they're now self-reporting kinds that may be authored by `"implementer"`, `"orchestrator"`, or `"user"`. `/p2e-ship-batch` enforces this via a scope-change audit at the end of Phase B (diff the as-implemented story against the briefed spec; surface unreported deltas before continuing to verify). `/p2e-work-on-next` relies on the implementer honoring the contract.
...
- **`workflows/p2e-work-on-next.md`** — the `### Human-authored kinds` section in the story-log policy is renamed `### Self-reporting kinds (implementer or human)`. `SCOPE_CHANGE` and `DECISION` may now be authored by `"implementer"`, `"orchestrator"`, or `"user"`; the orchestrator still never emits them automatically (the four orchestrator checkpoints in the existing policy are unchanged).
