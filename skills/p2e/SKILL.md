...
- requests about creating a new story -> read `workflows/p2e-add-story.md` AND `workflows/p2e-thicken.md`, then follow that workflow exactly (thick by default — all six thick-spec fields and graph context gathered before the preview; `--thin` opts out to fast placeholder capture)
...
- requests about implementing planned work -> read `workflows/p2e-work-on-next.md` AND `workflows/p2e-first-turn-briefing.md`, then follow the work-on-next workflow exactly (pick the next open story/stories and run the v2 supervisor; story-lead waves on Claude Code, sequential fallback on Codex/Cursor; stories end at IN_REVIEW, no release)
