...
| `--no-tracker` | Standalone mode only | Skip `criteria op=verdict` and `story_assets op=upload_url` writes. For dry-run / read-only UAT audits that must not mutate the tracker |
