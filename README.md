...
| Ship batch | `/p2e-ship-batch [release=v0.13] [phase=Build] [tag=plugin] [priority=P0,P1] [story_id=X-00-L0]… [--exclude=Y]… [--max=5] [--full-team] [--security \| --no-security] [--stop-on-fail] [--cut-release] [--budget=<td>] [--yes] [--dry-run]` | `p2e-ship-batch` / NL | `/p2e-ship-batch` / NL | Heavyweight batch ship — delegates per-story implementation to work-on-next (Phase B), then layers 360° verify via `/p2e-verify-story` (the v0.10.2 UAT-report workflow), per-story PR + `/pr-review-toolkit:review-pr`, conditional `/security-review` auto-triggered from diff paths (auth/secrets/oauth/permission/PII/migrations), and a rich-Markdown roll-up at `docs/feat-ship-batch-<date>/index.md`. Implementer-side spec deviations are captured as `SCOPE_CHANGE` / `DECISION` story-log entries via the shared first-turn briefing contract, and audited at the end of Phase B. Each story is tracked as a `TaskCreate` task with live status updates. Failure isolation defaults to skip-and-continue; `--stop-on-fail` halts the whole batch. Reach for this when shipping a release. |
