...
- On successful implementation the **story-lead** (or inline implementer on Codex/Cursor) runs gate steps 1–3 of the verify gate (`## Verify gate`): verificationCmd + consumer-impact sweep + risk-tiered review tool + adaptive fix loop. When the gate exits pass, the story-lead reports `outcome: "pass"` to the **supervisor**. The supervisor then executes the close-out steps: records per-AC verdicts via `mcp__p2e__criteria op=verdict` (with concrete evidence in the `note` field), writes the mandatory DEVIATIONS story-log entry (per `## Orchestrator DEVIATIONS checkpoint`), and flips the story to `IN_REVIEW`. The story-lead MUST NOT perform these close-out steps. For Fast/S/XS stories the PR is opened as part of the gate (after pre-PR review); for Standard/Architectural/Schema/Auth stories the PR is opened before the `pr-review-toolkit:review-pr` step and the story flips to `IN_REVIEW` after the review completes. The merge itself is outside the orchestrator (release is a separate, user-triggered concern).
...
4. **Gate steps 1–3 are run by the story-lead** (or the inline implementer on Codex/Cursor). When the gate exits pass (all findings addressed or triaged), the story-lead reports `outcome: "pass"` to the supervisor. **The following close-out steps are run by the SUPERVISOR** on receipt of a passing report (on sequential platforms — Codex/Cursor — the same session wears both hats, in this order): record per-AC verdicts via `mcp__p2e__criteria op=verdict` (see `## Story log checkpoint policy` Checkpoint 1 in `workflows/p2e-work-on-next.md`), write the DEVIATIONS story-log entry (see `## Orchestrator DEVIATIONS checkpoint`), then flip story to `IN_REVIEW`. The story-lead MUST NOT execute these close-out steps — they are supervisor responsibilities only.
