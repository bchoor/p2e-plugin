...
| Sync drift | `/p2e-sync <story_id>` | `p2e-sync` / NL | `/p2e-sync` / NL | On-demand field-level reconciliation between a P2E story and its linked GitHub issue body (four directions; Cherry-pick mode is Claude-only). |
| Manage UXO | `/p2e-manage-uxo [--edit <uxo_id>] [--add <uxo_id> --phase=<title> --tier=<name>] [--dry-run]` | `p2e-manage-uxo` / NL | `/p2e-manage-uxo` / NL | Edit (default) or add a UXO via the canonical writing recipe with an annotated preview/confirm gate. The preview shows which Flow + phase the UXO sits in. |
| Archaeology | `/p2e-archaeology [repo-path]` | `p2e-archaeology` / NL | `/p2e-archaeology` / NL | Autonomously onboard an existing repo — infer Flows/phases/UXOs, DONE layers from merged PRs, DRAFT stories from open gaps — no human interview. |
